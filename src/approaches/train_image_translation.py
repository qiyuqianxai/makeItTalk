...
class Image_translation_block():
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False, args=None):
        import time
        st = time.time()
        self.G.eval()
        ...
        # video writer: 输出文件名，输入源，帧率，图片尺寸（长，宽）
        for i, frame in enumerate(fls):
            ...
            for i in range(g_out.shape[0]):
                ...
                # frame = frame[:, 256:512, :]
                frame = np.array(g_out[i]) * 255.0

                # print(frame)
                # print(frame.shape, g_out[i].shape)
                # frame = g_out[i] * 255.0
                writer.write(frame.astype(np.uint8))
        ...
        if args is not None:
            os.system(
                'ffmpeg -loglevel error -y -i out.mp4 -i {} -pix_fmt yuv420p -strict -2 {}'.format(
